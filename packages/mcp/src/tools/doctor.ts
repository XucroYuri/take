...
export async function doctor(): Promise<DoctorReport> {
  ...
  return {
    ...
    imageProvider: { configured: hasKey('TAKE_IMAGE_API_KEY'), model: process.env.TAKE_IMAGE_MODEL ?? 'gpt-image-2' },
    videoProvider: { configured: hasKey('TAKE_VIDEO_API_KEY'), model: process.env.TAKE_VIDEO_MODEL ?? 'seedance-2.5' },
